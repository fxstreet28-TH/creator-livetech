/**
 * Email OTP delivery via Resend. Deno port of lib/email.ts.
 *
 * The Node original uses the `resend` npm SDK. Here we call the REST endpoint
 * the SDK wraps (POST https://api.resend.com/emails, Bearer auth, JSON body)
 * so the Edge Function pulls in no npm dependency for one request. The
 * observable contract — env vars, subject line, HTML body, result shape and
 * error strings — is unchanged.
 *
 * RESEND_FROM_EMAIL carries the display name too, e.g.
 * "AURUM Live <no-reply@creatorlivetech.com>".
 */
...
export async function sendEmailVerificationCode(
  ...
  try {
    ...
    if (!res.ok) {
      console.error('[resend] send failed', { status: res.status, data });
